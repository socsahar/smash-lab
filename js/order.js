...
async function handleOrderSubmit(e) {
    ...
    console.log('📦 Order data prepared (will be saved to DB after payment):', orderData);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        ...
        if (currentUser.name && currentUser.email) {
            console.log('User logged in, auto-filling form:', currentUser.name);
            ...
            if (window.orderDB && window.orderDB.getUserSavedWaiver) {
                window.orderDB.getUserSavedWaiver(currentUser.id).then(waiver => {
                    if (waiver && waiver.saved_waiver_data) {
                        console.log('✅ Found saved waiver for user');
                ...
                }).catch(err => console.log('No saved waiver found:', err));
            }
        }
    } catch (error) {
    ...
    try {
        ...
        console.log('Emoji removal skipped:', e);
    }
});
